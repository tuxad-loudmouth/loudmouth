...
/**
 * by togre 2017-04-22, see
 * https://github.com/mcabber/loudmouth/commit/792d8bd529f5dc0577dcc28c5f31b6a437d970fa
 */
static gboolean
...
ssl_verify_certificate (LmSSL *ssl, const gchar *server)
{
    ...
    if (X509_NAME_get_text_by_NID(crt_subj, NID_commonName, cn, LM_SSL_CN_MAX) > 0) {
        ...
        if (cn != NULL && ssl_match_domain_name(server, cn)) {
            ...
            g_log (LM_LOG_DOMAIN, LM_LOG_LEVEL_SSL, "%s: CN '%s' does not match server name '%s'\n", __FILE__, cn, server);
        }

    } else {
        g_log (LM_LOG_DOMAIN, LM_LOG_LEVEL_SSL,
               "X509_NAME_get_text_by_NID() NID_commonName failed");
    }
            ...
            for (san_counter=0; san_counter<subject_alt_names_nb; san_counter++) {
                ...
                if (current_name->type == GEN_DNS) {
                    ...
                    if (ssl_match_domain_name(server, (const char *)current_name->d.dNSName->data)) {
                        g_log (LM_LOG_DOMAIN, LM_LOG_LEVEL_SSL, "%s: found SAN '%s' - MATCH server '%s'\n", __FILE__, current_name->d.dNSName->data, server);
                        match_result = TRUE; /* break; */
                    } else {
                        g_log (LM_LOG_DOMAIN, LM_LOG_LEVEL_SSL, "%s: found SAN '%s' - no match server '%s'\n", __FILE__, current_name->d.dNSName->data, server);
                    }
                }
            }
